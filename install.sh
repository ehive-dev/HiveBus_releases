...
# HiveBus Installer/Updater (eHive One / Debian arm64)
#
# Stable:
...
[[ "$arch" == "$ARCH_REQ" ]] || fail "HiveBus benötigt ${ARCH_REQ}; erkannt wurde ${arch}."
...
info "Ermittle HiveBus-Release aus ${REPO} ..."
release="$(curl -fsSL --retry 3 -H 'Accept: application/vnd.github+json' "$api")" \
  || fail "Das HiveBus-Release konnte nicht geladen werden."
release_tag="$(printf '%s' "$release" | jq -r '.tag_name // empty')"
url="$(printf '%s' "$release" | jq -r --arg arch "$arch" '
  [.assets[]? | select(.name | test("^hivebus_[0-9].*_" + $arch + "\\.deb$"))]
  | .[0].browser_download_url // empty
')"
[[ -n "$release_tag" ]] || fail "Die Release-Antwort enthält keine Version."
[[ -n "$url" ]] || fail "Kein HiveBus-Paket für ${arch} in ${release_tag} gefunden."

tmp_dir="$(mktemp -d -t hivebus-install.XXXXX)"
trap 'rm -rf "$tmp_dir"' EXIT
deb_file="${tmp_dir}/hivebus.deb"

info "Lade HiveBus ${release_tag} ..."
...
fail "HiveBus wurde installiert, der Health-Check ist jedoch fehlgeschlagen."
